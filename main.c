...
void save_best()
{
	...
	for (n=0;n<9;n++)
		for (m=0;m<8;m++)
		{
			put_mem(30, 32768+(n*8)+m, bestlist[n].name[m]);
		}

	for (n=0;n<18;n+=2)
	{
		put_memw(30, 32768+72+n, bestlist[n/2].scoreboard);
	}

	load_file("bestlist", 30, 0);
...
void load_best()
{
	...
	load_file("bestlist", 30, 1);
		
	for (n=0;n<9;n++)
		for (m=0;m<8;m++)
		{
			bestlist[n].name[m]=get_mem(30, 32768+(n*8)+m);
		}

	for (n=0;n<18;n+=2)
	{
		bestlist[n/2].scoreboard=get_memw(30, 32768+72+n);
	}
}
...
// waiting for a space key
void press_start()
{
	while (TRUE)
	{
		vsync();
		if(joystick()||mouse_pos(&mouse_x,&mouse_y)==MOUSE_LBTN)
		{
	...
	switch (level)
		{
			...
			draw_image(17,9,IMG_MEDIUM_DISABLE);
			draw_image(17,13,IMG_HARD_DISABLE);
			draw_image(14,17,IMG_LIST_DISABLE);
			t_level=TIME_EASY_LEVEL;
			break;
			case 2:
			draw_image(17,5,IMG_EASY_DISABLE);
			draw_image(17,9,IMG_MEDIUM_ENABLE);
			draw_image(17,13,IMG_HARD_DISABLE);
			draw_image(14,17,IMG_LIST_DISABLE);
			...
			draw_image(17,5,IMG_EASY_DISABLE);
			draw_image(17,9,IMG_MEDIUM_DISABLE);
			draw_image(17,13,IMG_HARD_ENABLE);
			draw_image(14,17,IMG_LIST_DISABLE);
